...
/*   Updated: 2021/05/05 09:19:59 by jacher           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_second_init_userinput(t_input *user,
			t_term *term, t_list *log, char *prompt)
{
...
int		ft_handle_key_catching(t_input *user,
			t_term *term, char *prompt, t_list *log)
{
...
int		ft_get_userinput(char **line, char *prompt, char *log_path, int *ret)
{
	...
	*line = ft_read_input(g_msh.fd, &term, log, prompt);
	...
	if (*ret == -227 || *line == NULL || ft_update_log(line, &log, fd_log) < 0)
	{
		if (close(fd_log) < 0)
			*ret = -1;
		ft_lstclear(&log, free);
