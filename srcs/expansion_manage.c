...
/*   Updated: 2021/05/04 15:22:07 by jacher           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		manage_special_exit_help(t_expansion *exp)
{
	...
	exp->var_str = ft_itoa(g_msh.exit_status);
	if (exp->var_str == NULL)
...
int		manage_variable_help(t_expansion *exp, t_list *var, int k)
{
	...
	if (exp->mod == 1)
	{
		if ((r = find_variable_length(exp->str, exp->i + 1, k - 1, var)) == -1)
			return (-1);
		exp->j += r;
		exp->i = k;
	}
